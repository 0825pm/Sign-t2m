"""
LatentSignT2M Text-to-Motion Visualization
Based on SignGPT3 vis_lm_auto.py

Usage:
    cd ~/Projects/research/sign-t2m
    python vis_latent_t2m.py --checkpoint logs/light/runs/XXXX/checkpoints/last.ckpt \
...
# =============================================================================
# SOKE 120 dims format constants
# =============================================================================
...
def lm_forward(model, text, length, device, num_inference_steps=10, guidance_scale=4.0, gt_motion=None):
    """
    Forward pass through LatentSignT2M (Text → Motion).
    gt_motion: optional, for z distribution debugging
    """
    with torch.no_grad():
        # Generate
        feats_pred = model.generate([text], [length])
        
        # Debug: z distribution
        if gt_motion is not None:
            z_gt, _ = model.vae.encode(gt_motion, [length])
            z_gt = z_gt.squeeze()
            
            # Get z_pred by encoding generated motion
            z_pred, _ = model.vae.encode(feats_pred, [length])
            z_pred = z_pred.squeeze()
            
            z_mse = ((z_gt - z_pred) ** 2).mean().item()
            z_cos = torch.nn.functional.cosine_similarity(
                z_gt.flatten().unsqueeze(0), 
                z_pred.flatten().unsqueeze(0)
            ).item()
            
            print(f"  [z Debug] GT: mean={z_gt.mean():.3f}, std={z_gt.std():.3f} | "
                  f"Pred: mean={z_pred.mean():.3f}, std={z_pred.std():.3f} | "
                  f"MSE={z_mse:.4f}, Cos={z_cos:.4f}")
...
def main():
    ...
    # Parse Arguments
    # =========================
    parser = argparse.ArgumentParser(description='LatentSignT2M Text-to-Motion Visualization')
    ...
    # =========================
    print(f"\n{'='*70}")
    print("LatentSignT2M Text-to-Motion Visualization")
    print(f"{'='*70}")
    ...
    from src.models.latent_sign_t2m import LatentSignT2M
    from src.models.nets.text_encoder import CLIP
    
    # Load checkpoint first to get hparams
    ...
    model = LatentSignT2M(
        ...
        vae_checkpoint=hparams.get('vae_checkpoint', ''),
        vae_config=hparams.get('vae_config', {}),
        guidance_scale=args.guidance_scale,
        step_num=args.num_inference_steps,
        lambda_motion=hparams.get('lambda_motion', 0.5),
        use_motion_loss=hparams.get('use_motion_loss', True),
    )
    
    # Load state dict (includes z_mean, z_std buffers)
    ...
    # Check if z_stats are loaded
    if hasattr(model, 'z_mean') and model.z_mean.abs().sum() > 0:
        print(f"  z_mean loaded: mean={model.z_mean.mean():.4f}")
        print(f"  z_std loaded:  mean={model.z_std.mean():.4f}")
    else:
        print(f"  ⚠️  z_stats not initialized (old checkpoint?)")
    ...
    # =========================
    print(f"\n[Model Info]")
    print(f"  Model type: LatentSignT2M")
    print(f"  Feature dim: {SOKE_TOTAL_DIM}")
        ...
        for batch in tqdm(dataloader, desc=f"{split}"):
            ...
            with torch.no_grad():
                for i in range(B):
                    ...
                    gt_motion_for_debug = feats_ref[i:i+1, :length]
                    feats_pred = lm_forward(
                        model, text, length, device,
                        num_inference_steps=args.num_inference_steps,
                        guidance_scale=args.guidance_scale,
                        gt_motion=gt_motion_for_debug,
                    )
