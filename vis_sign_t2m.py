...
Usage:
    ...
    python vis_lm_light.py --checkpoint logs/light/runs/XXXX/checkpoints/last.ckpt \
...
def compute_metrics(gt_feats, recon_feats, gt_joints=None, recon_joints=None):
    ...
    # Part-wise feature metrics (SOKE 120-dim)
    if gt_feats.shape[-1] >= 120:
...
def lm_forward(model, text, length, device, num_inference_steps=10, guidance_scale=4.0):
    ...
    dummy_motion = torch.zeros(1, length, 120, device=device)
...
def main():
    ...
    datamodule = SignDataModule(
        ...
        mean_path='/home/user/Projects/research/SOKE/data/CSL-Daily/csl_mean_120.pt',
        std_path='/home/user/Projects/research/SOKE/data/CSL-Daily/csl_std_120.pt',
        batch_size=1,
        nfeats=120,
        njoints=55,
    ...
    for split in splits_to_process:
        ...
        for batch in tqdm(dataloader, desc=f"{split}"):
            ...
            texts = batch['text']
            lengths = batch['motion_len']  # Light-T2M uses motion_len
